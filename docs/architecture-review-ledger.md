...
- Dataset implementation is in progress. Versioning, the JSON path, and the one-sheet
  XLSX format adapter are complete; the current next checkpoint is XLSX integration in
  `dataset/loader.py`.
