...
| [ ] | Progress projection | Calculate completed Cases and Run progress from Results | `src/agentgate/application/result_reader.py` |
| [ ] | Activity projection | Return queued, running, and recent terminal Runs | `src/agentgate/application/result_reader.py` |
...
| [ ] | Customer scheduler | Accept work from an external Java scheduler | `src/agentgate/integrations/job_dispatchers/external.py` **new** |
...
| [ ] | Overview | Show Dataset and Run status statistics | `web/src/pages/Overview.vue` **new** |
| [ ] | Run console | Configure and submit evaluations | `web/src/pages/RunConsole.vue` **new** |
| [ ] | Run activity | Show queued, running, and historical Runs | `web/src/pages/RunActivity.vue` **new** |
| [ ] | Progress polling | Refresh status while queued or running work exists | `web/src/api/runs.ts` **new** |
| [ ] | Results | Show metrics, release gate, badcases, and traces | `web/src/pages/Results.vue` **new** |
| [ ] | Trace attribution | Display failure stages and supporting spans | `web/src/pages/TraceDetail.vue` **new** |
| [ ] | Evaluator management | Configure Rule, Judge, and Hybrid Evaluators | `web/src/pages/Evaluators.vue` **new** |
| [ ] | Skill analysis | Display Skill conflicts and prompt mismatches | `web/src/pages/SkillAnalysis.vue` **new** |
| [ ] | Optimizer | Display failure clusters and suggestions | `web/src/pages/Optimizer.vue` **new** |
...
| [ ] | A/B Web page | Display variants, differences, confidence, and winner | `web/src/pages/Comparison.vue` **new** |
...
| [ ] | Deterministic Skill checks | Detect duplicate names, Tool overlap, and missing descriptions | `src/agentgate/skill_analysis/rules.py` **new** |
| [ ] | Semantic Skill checks | Detect confusing descriptions and prompt mismatches | `src/agentgate/skill_analysis/judge.py` **new** |
