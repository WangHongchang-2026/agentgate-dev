...
version, Agent version, Skill version, or Evaluator version and see the related
evaluation records.
...
```text
             ...
             |         |         |
             v         v         v
        Dataset v3  Agent v2  Evaluators
             |         |
             v         v
        Case versions  Skill versions
```
...
The Web feature named **A/B Comparison** lets a user select two existing Evaluation Runs.
The backend compares them through `result/comparison.py`. It does not persist an A/B Test
or introduce an experiment lifecycle.

Each selected Run keeps its own lineage graph. The Web page may fetch both graphs and
display their shared and changed assets alongside the comparison result.
...
├── storage/
│   ├── repository.py             # descriptor and reverse-query contracts
...
LineageNodeKind = Literal[
    "run", "dataset", "case", "agent", "skill", "evaluator"
]
...
6. Deduplicate shared nodes and edges.
...
The A/B Comparison page remains owned by the Result Comparison feature. It may show two
Run lineage views, but lineage does not create or persist an A/B node.
...
7. Add transactional Run-asset indexes and reverse repository queries.
8. Add Dataset, Case, Target, Skill, and Evaluator lineage endpoints.
9. Implement Web visualization and browser tests.
10. Reconcile architecture, progress, and operational documentation.
...
- RunManifest, TargetDescriptor, DatasetVersion, and EvaluatorSpec are authoritative.
